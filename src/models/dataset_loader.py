...
class BikePerson(ImageDataset):
    ...
    def __init__(self, root='datasets', **kwargs):
        ...
        data_dir = osp.join(self.data_dir, 'BikePerson')
        if osp.isdir(data_dir):
            self.data_dir = data_dir
        else:
            warnings.warn(f'Directory {data_dir} not found.')

        train = lambda: self.process_dir(self.data_dir, DatasetType.Train)
        query = lambda: self.process_dir(self.data_dir, DatasetType.Query)
        gallery = lambda: self.process_dir(self.data_dir, DatasetType.Gallery)
    ...
    def extract_datasets(self, dir_path, dataset_type: DatasetType = DatasetType.Train):
        ...
        pattern = re.compile(r'cam_(/d)_(/d)/Bike/Person_(\d+)/cam(\d+)') 
        # 注：括号，指对正则表达式分组并记住匹配的文本

        data = []
        for img_path in img_paths:
            folder_camid_1, folder_camid_2, pid, camid = pattern.search(img_path).groups() # pid: person id, camid: camera id

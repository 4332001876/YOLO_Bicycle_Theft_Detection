...
class MySQLHelper():
    ...
    def load_data_to_mysql(self, table_name, data):
        ...
        self.test_connection()
        sql = "insert into " + table_name + " (milvus_id,bicycle_id,person_id) values (%s,%s,%s);"
        try:
    ...
    def insert(self, table_name, data):
        # Batch insert (Milvus_ids, img_path) to mysql
        self.test_connection()
        sql = "insert into " + table_name + " (milvus_id,bicycle_id,person_id,upload_status) values ('%s','%s','%s','%s',1);" % data
    ...
    def update(self, table_name, data):
        # Batch insert (Milvus_ids, img_path) to mysql
        self.test_connection()
        sql = "update " + table_name + " set milvus_id = %s,bicycle_id = '%s', person_id='%s' where id = %s;" % data
        n = self.cursor.execute(sql)
        self.conn.commit()
        LOGGER.debug(f"MYSQL loads data to table: {table_name} successfully")
        return n
    ...
    def search_by_milvus_ids(self, ids, table_name):
        ...
        sql = "select id,milvus_id,bicycle_id,person_id from " + table_name + " where milvus_id in (" + str_ids + ");"
        self.cursor.execute(sql)
    ...
    def search_by_ids(self, ids, table_name):
        ...
        self.test_connection()
        str_ids = str(ids).replace('[', '').replace(']', '')
        sql = "select id,milvus_id,bicycle_id,person_id from " + table_name + " where id in (" + str_ids + ");"
        self.cursor.execute(sql)
        results = self.cursor.fetchall()
        LOGGER.debug("MYSQL search by milvus id.")
        return results
    ...
    def search_by_update_status(self, table_name, upload_status, limit):
        # Get the img_path according to the milvus ids
        self.test_connection()
        sql = "select id,milvus_id,bicycle_id,person_id from %s where upload_status = %s limit %s " % (
